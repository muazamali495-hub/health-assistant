...
function hasKeyword(normText, keyword) {
  ...
  if (!k) return false;
  if (isAscii(k)) return new RegExp(`(^|\\s)${escapeRe(k)}(\\s|$)`).test(normText);
  if (normText.includes(k)) return true;

  // Urdu speakers slot extra words into a phrase ("گلے میں شدید درد" for the
  // keyword "گلے میں درد"), so an exact phrase match is too strict. For a
  // multi-word Urdu keyword, accept it when every part is present.
  const parts = k.split(' ');
